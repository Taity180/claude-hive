...
describe("RailSettingsPane", () => {
  ...
  it("offers all eight anchors", () => {
    render(<RailSettingsPane section="position" />);
  ...
  it("switches the resting form", async () => {
    render(<RailSettingsPane section="behaviour" />);
  ...
  it("lists muted apps and can unmute one", async () => {
    ...
    render(<RailSettingsPane section="muted" />);
  ...
  it("sets the window and sidebar opacity independently", async () => {
    render(<RailSettingsPane section="appearance" />);
  ...
  it("warns what hover means while Hive is in the rail", () => {
    // Leaving collapses the whole window there, which is a lot to happen
    // without having been told.
    useRailStore.setState({ combined: true });
    render(<RailSettingsPane section="behaviour" />);
    expect(screen.getByText(/collapses Hive to the nub/)).toBeInTheDocument();
  });
